...
|`druid.coordinator.kill.durationToRetain`|Only applies if kill is turned on. Note that the default value is invalid so you must provide an explicit config value if you set `druid.coordinator.kill.on` to  `true`. Do not kill unused segments whose interval end date is less than `now - durationToRetain`. durationToRetain can be negative, which would result in `now - durationToRetain` to be in the future.|`null`|
|`druid.coordinator.kill.ignoreDurationToRetain`|A way to override `durationToRetain` and tell the coordinator that you do not care about the end date of unused segment intervals when it comes to killing them. If true, the coordinator considers any and all unused segments are eligible to be killed.|false|
