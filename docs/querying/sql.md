...
|`REGEXP_REPLACE(expr, pattern, replace)`|Replaces a regular expression `pattern` applied to `expr` with `replace`|
|`REPLACE(expr, old_text, new_text)`|Replaces `old_text` with `new_text` in `expr`, and returns the result.|
